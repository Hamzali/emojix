...
# Sun Dec 14 18:21:46 +03 2025

Initially the work related to testing overhelmed me because it was all new concept plus was trying to test something already implemented but nevertheless it enabled me to find a bug and also changed strategy to make it both useful and easier to implement tests. My new startegy is implementing tests when you are introducing a new feature to a module so it let's me add purpose to the adding tests as well. So far I have discovered a lot of different ways to add mocks for my usecase tests however the best one I have found is creating a mock module under same module which can be overriedden. So far it works fine and not too verbose. Other thing I miss from nodejs/jest is having a standard expect calls to assert your test results. I have done something really similar at first however then I saw that I am losing the location of the failure in the test thus I have chnaged assertion helper functions to return result as error or nil then failed the test at test case body thus it saves a lot of repetition on writing the message of the assertion and check logic for large struct assertions. I am planning to introduce these helpers mostly local to each test especially struct specific ones but mocks are general and can be shared between different test suites.

As a last snapshot currently I have a solid way to test usecases, repositories and services. I am going bottom up because I want to write tests for things that I have defined, since I am not sure how the end UI would look like I haven't yet decided or written any e2e tests. However, my plan is to also introduce fuzzing, e2e and load tests in the future.
